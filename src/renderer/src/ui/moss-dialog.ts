  ...
  render() {
    return html`
      <sl-dialog
        ...
        style="${this.styles ? `${this.styles};` : ``}${this.width
          ? ` --width: ${this.width};`
          : ''}"
      >
  ...
  static styles = [
    ...
    css`
      ...
      .library-tool-details-dialog::part(panel) {
        ...
        left: clamp(
          100px,
          calc(100px + (100vw - 128px - min(var(--width, 1024px), calc(100vw - 128px))) / 2),
          calc(100vw - 28px - min(var(--width, 1024px), calc(100vw - 128px)))
        ) !important;
