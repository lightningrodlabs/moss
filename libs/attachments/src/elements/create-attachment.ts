...
export class CreateAttachment extends LitElement {
  ...
  renderMenuItems() {
    switch (this.appletsInfosAndGroupsProfiles.value.status) {
      case 'pending':
        return Array(3).map(
          () => html`<md-menu-item disabled><sl-skeleton></sl-skeleton></md-menu-item>`,
        );

      case 'complete':
        const { appletsInfos, groupsProfiles } = this.appletsInfosAndGroupsProfiles.value.value;

        const attachments = Array.from(this.weClient.attachmentTypes.entries());
        if (attachments.length === 0)
          return html`<md-menu-item disabled
            >${msg('There are no attachment types yet.')}</md-menu-item
          >`;

        const groupAttachmentTypes: Record<string, HoloHashMap<EntryHash, AttachmentType>> = {};

        for (const [appletHash, attachmentTypes] of attachments) {
          for (const [name, attachmentType] of Object.entries(attachmentTypes)) {
            ...
            if (!groupAttachmentTypes[stringifyedAttachmentType]) {
              groupAttachmentTypes[stringifyedAttachmentType] = new HoloHashMap();
            }
            groupAttachmentTypes[stringifyedAttachmentType].set(appletHash, attachmentType);
          }
        }
        ...
        return Object.entries(groupAttachmentTypes).map(
          ([stringifiedAttachmentType, attachmentTypesByApplet]) => {
            ...
            return html` <md-sub-menu-item .headline=${label}>
              <sl-icon slot="start" .src=${icon_src} style="margin-left: 16px"></sl-icon>
  ...
  render() {
    return html`
      ...
      <md-menu fixed id="menu" has-overflow> ${this.renderMenuItems()} </md-menu>
    `;
  }
