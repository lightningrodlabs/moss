...
import { BackgroundProcessorLifecycle, UnsubscribeFunction } from './types.js';

/**
 * Creates a lifecycle manager for a background processor.
 * This is a simple implementation that can be used by applets.
 */
export function createBackgroundProcessorLifecycle(
    ...
    // Track group active state - for now, default to true
    // This can be enhanced later to track actual group state
    const isGroupActive = readable<boolean>(true, (set) => {
        // TODO: Subscribe to active group changes
    ...
    const unsubscribeIsAppVisible = isAppVisible.subscribe((visible) => {
    ...
    const unsubscribeIsGroupActive = isGroupActive.subscribe((active) => {
        notifyLifecycleCallbacks({
    ...
    const unsubscribeResourceState = resourceState.subscribe((state) => {
        notifyLifecycleCallbacks({
            isAppVisible: get(isAppVisible),
            isGroupActive: get(isGroupActive),
            resourceState: state,
