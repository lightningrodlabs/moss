...
export function setupLogs(launcherEmitter: LauncherEmitter) {
  ...
  launcherEmitter.on(HOLOCHAIN_LOG, (holochainData) =>
    console.log(
      `[HOLOCHAIN ${(holochainData as HolochainData).version}]: `,
      (holochainData as HolochainData).data,
    ),
  );
  launcherEmitter.on(HOLOCHAIN_ERROR, (holochainData) =>
    console.log(
      `[HOLOCHAIN ${(holochainData as HolochainData).version}] ERROR: ${
        (holochainData as HolochainData).data
      }`,
    ),
  );
  launcherEmitter.on(WASM_LOG, (holochainData) =>
    console.log(
      `[HOLOCHAIN ${(holochainData as HolochainData).version}] WASM: ${
        (holochainData as HolochainData).data
      }`,
    ),
  );
