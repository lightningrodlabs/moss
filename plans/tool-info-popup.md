...
- Other potential sites (personal sidebar, topbar, group home grid, group settings,
  tool library grid) are deliberately deferred — several of them live in code paths
  that the [cruft cleanup plan](ui-testing-and-cruft-cleanup.md) is expected to
  delete or restructure. Wiring right-click into doomed components now is wasted
  work; revisit per-site after that cleanup lands.
...
### E2E (Playwright harness — already in repo)

Add to `tests/e2e/smoke/`:
