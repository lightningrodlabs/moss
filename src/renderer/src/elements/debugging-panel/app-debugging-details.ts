...
export class AppDebuggingDetails extends LitElement {
  ...
  renderGossipForDna(dnaHashB64: DnaHashB64, metrics: NetworkMetrics) {
    ...
    const peerEntries = Object.entries(gossip.peer_meta).sort(([, a], [, b]) => {
      const aTime = a.last_gossip_timestamp ?? 0;
      const bTime = b.last_gossip_timestamp ?? 0;
      return bTime - aTime;
    });

    const totalCompleted = peerEntries.reduce((sum, [, pm]) => sum + (pm.completed_rounds ?? 0), 0);
    const totalErrors = peerEntries.reduce(
      (sum, [, pm]) => sum + (pm.peer_behavior_errors ?? 0) + (pm.local_errors ?? 0),
      0,
    );
    const totalTimeouts = peerEntries.reduce((sum, [, pm]) => sum + (pm.peer_timeouts ?? 0), 0);

    return html`
      <div style="margin: 8px 0; padding: 8px; background: #f0f4ff; border-radius: 4px;">
        <div
          style="font-size: 11px; margin-bottom: 6px; display: flex; gap: 12px; flex-wrap: wrap;"
        >
          <span>Rounds: <b>${initiatedCount}</b> init, <b>${acceptedCount}</b> accept</span>
          <span
            >Fetch: <b>${pendingFetchCount}</b> pending${backoffCount > 0
              ? html`, <span style="color: #c44;">${backoffCount} backoff</span>`
              : ''}</span
          >
          ...
          ${totalTimeouts > 0
            ? html`<span style="color: #c44;">T/O: ${totalTimeouts}</span>`
            : html``}
        </div>
                ...
                <tbody>
                  ${peerEntries.map(([peerUrl, pm]) => {
                    const lastGossip = pm.last_gossip_timestamp
                      ...
                      : 'never';
                    const hasErrors =
                      (pm.peer_behavior_errors ?? 0) > 0 || (pm.local_errors ?? 0) > 0;
                    const hasTimeouts = (pm.peer_timeouts ?? 0) > 0;

                    return html`
                      <tr style="border-bottom: 1px solid #ddd;">
                        <td style="padding: 2px 4px; font-family: monospace;" title="${peerUrl}">
                          ${extractPeerShortId(peerUrl)}
                        </td>
                        <td style="padding: 2px 4px;">${lastGossip}</td>
                        <td style="padding: 2px 4px;">${pm.completed_rounds ?? 0}</td>
                        <td style="padding: 2px 4px; ${hasErrors ? 'color: #c44;' : ''}">
                          ${(pm.peer_behavior_errors ?? 0) + (pm.local_errors ?? 0)}
                        </td>
                        <td style="padding: 2px 4px;">${pm.peer_busy ?? 0}</td>
                        <td style="padding: 2px 4px; ${hasTimeouts ? 'color: #c44;' : ''}">
                          ${pm.peer_timeouts ?? 0}
                        </td>
        ...
        ${pendingFetchCount > 0 || backoffCount > 0
          ? html`
              <div
              ...
              >
                <span
                  style="display: inline-block; transition: transform 0.2s; transform: rotate(${this._expandedFetchQueues.has(
                    dnaHashB64,
                  )
                    ? '90deg'
                    : '0deg'});"
              ...
              ${this._expandedFetchQueues.has(dnaHashB64)
                ? html`
                    <div style="margin-top: 4px; padding-left: 12px; font-size: 11px;">
                      ${pendingFetchCount > 0
                        ? html`
                            <div style="margin-bottom: 4px;">
                              <b>Pending (${pendingFetchCount}):</b>
                            </div>
                            ${Object.entries(fetch.pending_requests)
                              .slice(0, 20)
                              .map(
                                ([opHash, peerUrls]) => html`
                                  <div
                                    style="padding-left: 8px; margin-bottom: 2px; font-family: monospace;"
                                  >
                                    ${opHash.length > 16 ? `${opHash.slice(0, 12)}...` : opHash}
                                    <span style="color: #666;">
                                      from ${(peerUrls as string[]).length} peer(s)</span
                                    >
                                  </div>
                                `,
                              )}
                            ${pendingFetchCount > 20
                              ? html`<div style="color: #999;">
                                  ...and ${pendingFetchCount - 20} more
                                </div>`
                              : html``}
                          `
                        : html``}
                      ${backoffCount > 0
                        ? html`
                            ...
                            ${(fetch.peers_on_backoff instanceof Map
                              ...
                              return html` <div style="padding-left: 8px; font-family: monospace;">
                                ${extractPeerShortId(String(url))}
                                <span style="color: #666;">
                                  expires ${new Date(expiryNum / 1000).toLocaleTimeString()}</span
                                >
                              </div>`;
                            })}
                          `
            ...
            <sl-card style="margin: 5px 0;">
              <div class="column" style="gap: 0;">
                <div style="font-weight: bold; margin-bottom: 4px;">
                  ${cellName}
                  <span
                    style="font-weight: normal; font-size: 11px; color: #666; margin-left: 8px;"
                    title="${dnaHashB64}"
                    >${dnaHashB64.slice(0, 6)}...${dnaHashB64.slice(-4)}</span
                  >
                </div>
