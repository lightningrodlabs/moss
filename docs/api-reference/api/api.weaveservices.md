...
[captureAudioSources?](./api.weaveservices.captureaudiosources.md)
...
(opts?: [CaptureAudioSourcesOptions](./api.captureaudiosourcesoptions.md)<!-- -->) =&gt; Promise&lt;[AudioSourceCapture](./api.audiosourcecapture.md) \| null&gt;


</td><td>

_(Optional)_ Asks the host for audio playing on the user's machine. The host shows its own picker. Resolves `null` when the user declines or the host cannot capture. Absent on hosts without the feature — feature-detect with `client.captureAudioSources?.`<!-- -->.
