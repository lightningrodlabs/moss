...
export class NotificationCard extends LitElement {
  ...
  private async processBodyWithAgentNames(body: string): Promise<string> {
    ...
    const agentNamesAndIcons = await Promise.all(
      agentKeys.map((key) => this.getAgentNameAndIcon(key)),
    );
  ...
  private async getAgentNameAndIcon(pubkeyB64: string): Promise<Array<string>> {
    try {
      ...
      if (profileStore && profileStore.type === 'profile') {
        return [
          profileStore.profile.entry.nickname || pubkeyB64.slice(0, 8),
          profileStore.profile.entry.fields.avatar || '',
        ];
      }
  ...
  render() {
    ...
    const fromAgentKey = this.notification?.fromAgent
      ? encodeHashToBase64(this.notification.fromAgent)
      : undefined;
    ...
    switch (this.appletLogo.value.status) {
      case 'pending':
        return html``;
      case 'complete':
        return html` <div class="column notification-card">
              ...
              ? until(
                  this.getAgentNameAndIcon(fromAgentKey).then(([name, icon]) =>
                    icon
                      ...
                      : html`<div class="profile-img" title=${name}>
          ...
          <div class="notification-center">
            ...
            <!-- ${fromAgentKey
              ? until(
                  this.getAgentNameAndIcon(fromAgentKey).then(
                    ([name, _icon]) => html`from <b>${name}</b>`,
                  ),
                ...
                ? html`
                    <sl-tooltip content=${msg('Open asset in sidebar')} placement="left">
                      <button
                        ...
                        @click=${() => {
                          console.log(
                            'Dispatching open-wal event for notification:',
                            this.notification,
                          );
                          this.dispatchEvent(
