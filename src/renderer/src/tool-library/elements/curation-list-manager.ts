...
export class UrlListManager extends LitElement {
  ...
  private async _doFactoryReset() {
    ...
    this.dispatchEvent(new CustomEvent('urls-changed', { detail: this._urls, bubbles: true }));
  }
  ...
  async initializeList(urls: string[]) {
    ...
    for (const cur of urls) {
      try {
        const url = cur.startsWith('http') ? cur : `https://${cur}`;
  ...
  private async _addUrl() {
    ...
    let url: URL;
    try {
      url = new URL(this._newUrl.startsWith('http') ? this._newUrl : `https://${this._newUrl}`);
    } catch {
      this._error = 'url';
      return;
    }
        ...
        <div class="publish-info">
          ${msg(
            html`Learn more about
              <a
                href="https://dev.theweave.social/build/publishing-a-tool.html"
                target="_blank"
                ...
                return html`
                  <div style="display:flex; flex-direction:row; align-items:center; gap:8px;">
                    ...
                    <sl-tooltip content=${msg('Remove curation list')} placement="left">
                      <button class="open-wal-button" @click=${() => this._removeUrl(u.id)}>
                        ${trashIcon()}
                      </button>
                    </sl-tooltip>
                  </div>
                ...
                >
                  ...
                  ${this._confirmingReset
                    ? html`
                        ...
                        <sl-button variant="danger" @click=${() => this._doFactoryReset()}>
                          ${msg('Confirm Reset')}
                        </sl-button>
                      `
                    : html`
