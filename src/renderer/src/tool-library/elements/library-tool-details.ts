...
export class LibraryToolDetails extends LitElement {
  ...
  renderVersions() {
    if (this.unifiedTool) {
      ...
      const branches = Array.from(this.unifiedTool.versionBranches.values()).sort((a, b) => {
              ...
              ${displayTool.tags.length > 0
                ? html`
                    <div class="row tool-tag-list" style="margin-top:6px">
                      ${displayTool.tags.map((tag) => html`<div class="tool-tag">${tag}</div>`)}
                    </div>
                  `
                : ''}
            </div>
              ...
              : primaryBranch
                ? html`
                    <select-group
                      .buttonWidth=${'auto'}
                      .buttonText=${msg(
                        str`Install v${primaryBranch.latestVersion.version} to a group space`,
                      )}
                      @group-selected=${async (e: CustomEvent) => {
                        this.dispatchEvent(
                          new CustomEvent('install-tool-to-group', {
                ...
                : this.tool
                  ? html`
                      <select-group
                        ...
                        .buttonText=${(() => {
                          if (
                            this.tool?.toolInfoAndVersions.versions &&
                            this.tool.toolInfoAndVersions.versions.length > 0
                          ) {
                    ...
                    `
                  : ''}
            ${!this.informational && hasMultipleBranches
              ? html`
                  <div
                    style="font-size: 12px; color: rgba(0, 0, 0, 0.4); margin-top: -15px; text-align: right;"
                  >
