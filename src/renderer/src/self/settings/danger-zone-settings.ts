...
import {
  GroupImportResult,
  ImportGroupsProgress,
  NetworkOverridesInfo,
    ...
    return html`
      <div class="column">
        <div>
          ...
          <sl-button
            ...
            @click=${async () => {
              ...
              try {
                ...
                alert(
                  msg('An error occurred during export. Please check the console for details.'),
                );
              } finally {
          ...
          <sl-button
            ...
            @click=${async () => {
              ...
              try {
                ...
                alert(
                  msg('An error occurred during import. Please check the console for details.'),
                );
              } finally {
          ...
          ${this.isImporting && this.importProgress
            ? html`
                <div
                  style="margin-top: 12px; padding: 10px; background: #f0f0f0; border-radius: 8px; font-size: 13px;"
                >
                  ...
                  <div>
                    ...
                    ${this.importProgress.step === 'installing'
                      ? msg('Installing...')
                      : this.importProgress.step === 'setting-profile'
                        ? msg('Setting group profile...')
                        : this.importProgress.step === 'waiting-for-sync'
                          ? html`${msg('Waiting for data to sync...')}
                            (${this.importProgress.secondsLeft}s)`
                          : this.importProgress.step === 'installing-tool'
                            ? html`${msg('Installing tool')} "${this.importProgress.toolName}"
                              (${this.importProgress.toolIndex}/${this.importProgress.toolTotal})...`
                            : ''}
                  </div>
                </div>
              `
            : ''}
          ${this.importResults && !this.isImporting
            ? html`
                <div
                  style="margin-top: 12px; padding: 10px; background: #f0f0f0; border-radius: 8px; font-size: 13px;"
                >
                    ...
                    (r) => html`
                      <div style="margin-bottom: 4px;">
                        ...
                        ${r.status === 'created'
                          ...
                          : r.status === 'joined'
                            ? msg('Joined')
                            : r.status === 'joined-no-profile'
                              ? msg('Joined (profile not yet synced)')
                ...
                <div class="column" style="gap: 12px; margin-top: 12px;">
                  ...
                  >
                    <span style="flex: 1; font-size: 13px; color: #664d03;">
                      ${msg(
                        'Changing these settings will relaunch Moss. All peers must use the same bootstrap server to connect.',
                      )}
                    </span>
